...
class LoggerManager:
    ...
    async def log_conversation(self, session_id: str, user_id: str, conversation_data: Dict):
        """记录完整对话"""
        try:
            timestamp = datetime.now()
    ...
    async def log_chat_event(self, event_type: str, session_id: str = None, user_id: str = None, 
                           message_content: str = None, duration: float = None):
        """记录聊天事件"""
        try:
            timestamp = datetime.now()
    ...
    async def log_performance(self, operation: str, duration: float, details: Optional[Dict] = None):
        """记录性能信息"""
        try:
            timestamp = datetime.now()
    ...
    async def log_rag_query(self, session_id: str, query: str, retrieved_docs: int, 
                           processing_time: float, has_references: bool):
        """记录RAG查询信息"""
        try:
            timestamp = datetime.now()
    ...
    async def log_websocket_event(self, session_id: str, event_type: str, 
                                user_id: str = None, message_size: int = 0):
        """记录WebSocket事件"""
        try:
            timestamp = datetime.now()
